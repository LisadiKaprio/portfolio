...
<template>
    <NuxtLink v-if="item" :to="`${item._path}`">
        ...
        <VideoComponent
            height="200px"
            width="100%"
            :mp4-file-name="item.coverMp4"
            :webm-file-name="item.coverWebm"
            :image-file-name="item.coverImage"
        ...
        />
            ...
            <v-chip v-for="skill in item.skills" :key="skill" size="small" class="mr-2">
                {{ skill }}
            </v-chip>
        </v-card>
    </NuxtLink>
